...
const adminState = data => ({
  ...
  usageSummary: summarizeUsageEvents(data.usageEvents),
...
async function renderUsage(){var eid=$('usageEmployeeFilter').value;var mid=$('usageModelFilter').value;var query=new URLSearchParams({page:String(usagePage),pageSize:String(usagePageSize)});if(eid)query.set('employeeId',eid);if(mid)query.set('modelId',mid);var report=await api('/api/admin/usage?'+query.toString());usagePage=report.page;var start=(report.page-1)*report.pageSize;var pageRows=report.rows||[];$('usageCalls').textContent=report.callCount;$('usageCredits').textContent=Number(report.creditsUsed||0).toFixed(4);$('usageMoney').textContent='-';$('usageRows').innerHTML=pageRows.map(function(r){var emp=state.employees.find(function(e){return e.employeeId===r.employeeId;});return '<tr><td>'+esc(r.day)+'</td><td>'+esc(emp?emp.employeeName:r.employeeId)+'<br><code>'+esc(r.employeeId)+'</code></td><td>'+esc(r.modelId)+'</td><td>'+r.tokens+'</td><td>'+Number(r.credits||0).toFixed(4)+'</td><td>'+esc(money(r.credits,r.modelId))+'</td></tr>';}).join('');$('usagePagerInfo').textContent=report.totalRows?'第 '+report.page+' / '+report.pageCount+' 页，共 '+report.totalRows+' 条，当前显示 '+(start+1)+'-'+Math.min(start+pageRows.length,report.totalRows):'暂无用量记录';$('usagePrevPage').disabled=report.page<=1;$('usageNextPage').disabled=report.page>=report.pageCount;}
...
$('usageEmployeeFilter').onchange=function(){usagePage=1;run(renderUsage);};
$('usageModelFilter').onchange=function(){usagePage=1;run(renderUsage);};
$('usagePrevPage').onclick=function(){usagePage=Math.max(1,usagePage-1);run(renderUsage);};
$('usageNextPage').onclick=function(){usagePage+=1;run(renderUsage);};
...
const handleAdmin = async (req, res, url, data) => {
  ...
  if (url.pathname === '/api/admin/usage' && req.method === 'GET') {
    return ok(res, buildUsageReport(data.usageEvents, {
