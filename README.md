...
当前只有企业 MCP `dz2.0`（兜知2.0）启用楼层权限，其他企业 MCP 继续沿用原有“是否授权 MCP”的控制方式，不显示楼层选项。

楼层权限包括营运一层、营运二层、营运三层、营运四层和营运自营。

- 每名员工可独立多选楼层；个人配置优先于部门继承权限。
- 历史员工或部门已经拥有 `dz2.0`、但没有楼层明细时，默认视为五项全选，保证旧授权继续可用。
- 管理后台的员工授权和有效授权弹窗会展示最终楼层权限，并支持人员、部门模糊筛选。
- 客户端“企业激活”页展示当前员工的兜知2.0最终楼层权限。
- 企业服务通过稳定的短期 HMAC 签名请求头把权限交给 MCP 服务；只有权限实际变化才需要同步网关配置，定时刷新不会持续重启网关。
- MCP 服务保留原飞书机器人调用链路；缺少新签名时不进入 LFCLAW 楼层权限分支，避免影响既有接入。

企业服务相关代码位于 `enterprise-server/server.mjs` 和 `enterprise-server/mcpPermissions.mjs`。MCP 服务端还需要同步部署对应的楼层验权版本，否则客户端虽然能展示权限，查询范围不会真正受限。
...
Windows 上推荐直接双击仓库根目录的 `发布Windows安装包.cmd`，不需要手工执行命令。该脚本会：

- 自动选择 Node.js `24.15.0+`；本机没有兼容版本时，在 `.release-tools` 准备项目专用便携版 Node，不修改系统环境。
- 只关闭从当前仓库启动的 Node、Electron 和 esbuild 开发进程，避免 `better_sqlite3.node` 被占用。
- 优先复用 `node_modules/electron/dist` 中版本匹配的 Electron，避免 GitHub 下载超时；本地运行时不可用时再走 electron-builder 默认下载流程。
- 调用正式发布命令 `npm run release:win`，生成版本号、更新日志、安装包和 SHA256，并在成功后打开 `release` 目录。

打包过程中不要关闭窗口。首次准备便携版 Node 或压缩 OpenClaw、技能和 Python 运行时时间会更长。
