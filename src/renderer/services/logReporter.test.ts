...
test('builds a Enterprise analytics URL with common action parameters', () => {
  const result = new URL(buildLogUrl(
    {
      action: `${LogReporterActionPrefix.LfClaw}skill_enabled`,
      skillId: 'xlsx',
      enabled: true,
    },
    {
  ...
  expect(result.origin + result.pathname).toBe(LogReporterEndpoint.Enterprise);
  expect(result.searchParams.get('_npid')).toBe(LogReporterProduct.LfClaw);
  ...
  expect(result.searchParams.get('action')).toBe('lfclaw_skill_enabled');
...
test('uses the logged-in user and omits empty optional parameters', () => {
  const result = new URL(buildLogUrl(
    {
      action: `${LogReporterActionPrefix.LfClaw}app_started`,
...
test('reports an event through the Electron API bridge', async () => {
  ...
  expect(requestUrl.searchParams.get('action')).toBe('lfclaw_plan_mode_enabled');
...
test('rejects an event without the LfClaw action prefix before sending', async () => {
  const fetchMock = vi.fn();
  vi.stubGlobal('window', {
    electron: {
      api: {
